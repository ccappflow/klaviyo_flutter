...
class KlaviyoMethodCallHandler: MethodCallHandler {
    companion object {
        private const val CHANNEL_NAME = "com.rightbite.denisr/klaviyo"
        private var instance: KlaviyoMethodCallHandler? = null

        fun getInstance(): KlaviyoMethodCallHandler {
            if (instance == null) {
                instance = KlaviyoMethodCallHandler()
            }
            return instance!!
        }
    }
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        ...
        when (call.method) {
            ...
            METHOD_UPDATE_PROFILE -> {
                try {
                    ...
                    var profileProperties =
                        convertMapToSerializedMap(
                            profilePropertiesRaw
                        )
            ...
            METHOD_GET_EXTERNAL_ID -> result.success(
            ...
            METHOD_GET_EMAIL -> result.success(
                Klaviyo.getEmail())
            METHOD_GET_PHONE_NUMBER -> result.success(
            ...
            METHOD_REQUEST_TOKEN -> {
                FirebaseMessaging.getInstance().token.addOnCompleteListener { task ->
                    if (!task.isSuccessful) {
                        ...
                        KlaviyoEventChannelHandler.getInstance().onTokenChanged(token)
...
private fun convertMapToSerializedMap(map: Map<String, Any>): Map<String, Serializable> {
